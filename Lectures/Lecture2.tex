...
A DFA decides some property $P(x)$ on strings $x \in \Sigma^*$.
...
\begin{itemize}
    ...
    \item The reversal of of a regular language is regular
    \item The closure (star) of a regular language is regular
    \item The concatenation of regular languages is regular
    \item A homomorphism of a regular language is regular
    \item The inverse homomorphism of a regular language is regular
\end{itemize}

\paragraph{Clarification (homomorphisms):} A homomorphism on a language $L \subseteq \Sigma^*$ is a function $h$ on $\Sigma$ to some other alphabet $\Gamma$ s.t $h: \Sigma \rightarrow \Gamma^*$. In essence, a replacement of symbols for strings. The inverse homomorphism of some language $S$ is simply $h^{-1}(S) = \setdef{w \in \Sigma^*}{h(w) \in S}$.

\textit{Proofs when time...}
